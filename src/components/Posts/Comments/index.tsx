...
const Comments: React.FC<CommentsProps> = ({
  ...
  const isDeadline: boolean =
    new Date(topicStateValue.currentTopic?.final_closure_date).getTime() >
    new Date().getTime();
  ...
  const onCreateComment = async (comment: string) => {
    ...
    try {
      ...
      axios
        .post('https://backend-2tza.onrender.com/idea/comment', {
          comment: comment,
          client_id: user.uid,
        ...
        .then((res) => {
          ...
          let updatedComments = JSON.parse(
            JSON.stringify(ideaStateValue.Ideas)
          );
          updatedComments[ideaStateValue.selectedIdeaIndex].comments.push(
            res.data
          );
